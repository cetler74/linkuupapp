...
const GroupedBookingList: React.FC<GroupedBookingListProps> = ({
  bookings,
  employees,
  onAccept,
  onDecline,
  ...
  const groupedBookings = useMemo(() => {
    ...
    if (laterBookings.length > 0) {
      ...
      laterGroups.forEach((groupBookings, weekKey) => {
        ...
        groups.push({
          label: `${label} - ${new Date(weekKey).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`,
          date: weekKey,
      ...
      {groupedBookings.map((group) => (
        <View key={group.date} style={styles.group}>
          <View style={styles.groupHeader}>
            ...
            <Text style={styles.groupCount}>({group.bookings.length})</Text>
          </View>
