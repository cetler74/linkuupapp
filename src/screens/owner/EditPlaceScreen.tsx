...
import { ownerAPI, type Place, type PlaceImage, getImageUrl } from '../../api/api';
...
const EditPlaceScreen = () => {
  ...
  const loadPlaceData = async () => {
    try {
      setIsLoading(true);
      const loadedPlace = await ownerAPI.getPlace(placeId);
      setPlace(loadedPlace);
      
      const latitude = loadedPlace.latitude || null;
      const longitude = loadedPlace.longitude || null;
      
      setFormData({
        name: loadedPlace.nome || '',
        sector: loadedPlace.tipo || '',
        description: loadedPlace.about || '',
        address: loadedPlace.rua || '',
        city: loadedPlace.cidade || '',
        postal_code: loadedPlace.cod_postal || '',
        phone: loadedPlace.telefone || '',
        email: loadedPlace.email || '',
        booking_enabled: loadedPlace.booking_enabled ?? true,
        latitude: latitude,
        longitude: longitude,
      });
      ...
      if (loadedPlace.images) {
        setExistingImages(loadedPlace.images);
      }
    } catch (error: any) {
      console.error('Error loading place:', error);
      Alert.alert(
        t('common.error') || 'Error',
        error.message || t('places.loadError') || 'Failed to load place data.'
      );
      navigation.goBack();
    } finally {
      setIsLoading(false);
  ...
  const removeExistingImage = async (imageId: number) => {
    try {
      await ownerAPI.deletePlaceImage(imageId);
      setExistingImages(existingImages.filter((img) => img.id !== imageId));
      Alert.alert(
        t('common.success') || 'Success',
        t('places.imageDeleted') || 'Image deleted successfully'
      );
    } catch (error: any) {
      console.error('Error deleting image:', error);
      Alert.alert(
        t('common.error') || 'Error',
        error.message || t('places.imageDeleteError') || 'Failed to delete image.'
      );
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const placeData: any = {
        ...
        is_active: place?.is_active !== false, // Preserve existing status or default to true
        pais: 'Portugal',
      ...
      await ownerAPI.updatePlace(placeId, placeData);
      
      // Upload new images if any were selected
      if (selectedImages.length > 0) {
        try {
          setUploadingImages(true);
          await ownerAPI.uploadPlaceImages(placeId, selectedImages);
          setSelectedImages([]);
        } catch (imageError) {
          console.error('Error uploading images:', imageError);
          Alert.alert(
            t('common.warning') || 'Warning',
            t('places.placeSavedButImagesFailed') || 'Place updated but failed to upload images. You can add them later.'
          );
        } finally {
      ...
      Alert.alert(
        t('common.success') || 'Success',
        t('places.placeUpdated') || 'Place updated successfully!',
        [
          {
      ...
      Alert.alert(
        t('common.error') || 'Error',
        error.message || t('places.updateError') || 'Failed to update place. Please try again.'
      );
    } finally {
        ...
        <Card style={styles.formCard}>
          ...
          {existingImages.length > 0 && (
            <View style={styles.existingImagesContainer}>
              <Text style={styles.imagePreviewLabel}>
                {t('places.currentImages') || 'Current Images'}
              </Text>
          ...
          <TouchableOpacity onPress={pickImages} style={styles.imagePickerButton}>
            ...
            <Text style={styles.imagePickerText}>
              {t('places.addingNewImages') || 'Add New Images'}
            </Text>
          </TouchableOpacity>
            ...
            <Button
              title={
                uploadingImages
                  ...
                  : isSubmitting
                  ? t('places.uploading') || 'Updating...'
                  : t('places.update') || 'Update Place'
              }
